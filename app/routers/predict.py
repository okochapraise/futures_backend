...
from app.utils.db import insert_signal  # ✅ NEW: import DB logging
import os
import traceback
...
async def get_prediction(pair: str = Query(..., description="Trading pair e.g., SOLUSDT")):
    try:
        ...
        # ✅ Handle invalid/unknown pairs gracefully
        ...
        # 📧 Send email if confidence is high enough
        if confidence >= 70:
            email_body = f"""
🚨 Signal Alert for {pair} 🚨

🔹 Action: {action.upper()}
🔹 Confidence Score: {confidence}%
🔹 Market Order: {action} at ${entry_price:.2f}
🔹 Take Profit: ${take_profit if take_profit else 'N/A'}
🔹 Stop Loss: ${stop_loss if stop_loss else 'N/A'}

📊 Reasons:
{chr(10).join(f"• {r}" for r in reason)}
...
"""
            await send_email(
                subject=f"📈 {action} signal for {pair}",
                body=email_body,
                to=os.getenv("ALERT_EMAIL")
            )

            # ✅ Log high-confidence signal to database
            insert_signal({
                "pair": pair,
                "signal": action,
